...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		keysArray = [for (i in 0...SONG.mania + 1) '${SONG.mania}_key_$i'];
		...
		timeTxt.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
	...
	private function generateSong():Void
	{
		...
		for (section in sectionsData)
		{
			...
			for (i in 0...section.sectionNotes.length)
			{
				...
				var noteColumn:Int = Std.int(songNotes[1] % (SONG.mania + 1));
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		// strumLineY += ManiaData.noteOffsetsY[SONG.mania];
						...
						while (i < notes.length)
						{
							...
							if (Conductor.songPosition - daNote.strumTime > noteKillOffset)
							{
								if (daNote.mustPress
									...
									&& !(daNote.isSustainNote && daNote.animation.curAnim.name.endsWith("end")))
									noteMiss(daNote);
	...
	function noteMissCommon(direction:Int, note:Note = null)
	{
		...
		if (char != null && (note == null || !note.noMissAnimation) && char.hasMissAnimations)
		{
			...
			var animToPlay:String = "sing" + ManiaData.noteAnimations[SONG.mania][note.noteData].toUpperCase() + 'miss' + postfix;
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		{
			var char:Character = dad;
			var animToPlay:String = "sing" + ManiaData.noteAnimations[SONG.mania][note.noteData].toUpperCase() + note.animSuffix;
			if (note.gfNote)
				char = gf;
